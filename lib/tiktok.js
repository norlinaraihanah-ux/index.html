...
//
// Each endpoint path is overridable via env in case TikTok bumps
// the version. We never throw upward — the dashboard prefers
// degraded data over a 500.
// ============================================================
...
// Music — trending songs (same discovery endpoint, discovery_type=SONG)
const MUSIC_TRENDING_URL = process.env.TIKTOK_MUSIC_TRENDING_URL
  || 'https://sandbox-ads.tiktok.com/open_api/v1.3/discovery/trending_list/';

// Creative Recommendation — top creatives (same discovery endpoint, discovery_type=INDUSTRY)
const CREATIVE_TOP_ADS_URL = process.env.TIKTOK_CREATIVE_TOP_ADS_URL
  || 'https://sandbox-ads.tiktok.com/open_api/v1.3/discovery/trending_list/';

/**
 * Internal — call a TikTok endpoint with the env-supplied token.
 * Returns parsed JSON or an error envelope on failure (we never
 * throw upward; the dashboard prefers degraded data over a 500).
 */
 ...
 * Returns { rows, error, debug } so the endpoint can surface what
 * TikTok actually responded with (status, message, sample of body).
 */
